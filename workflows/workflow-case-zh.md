...
> 原文：`workflows/workflow-case.json`（caseVersion 1，caseKind `illustrative-completed-run`·示例性已完成运行）
> 技能名称对照：`workflows/skills-list.json`（目录中 38 个可用技能）

| 字段 | 值 |
|---|---|
| 案例 ID | `async-cancellation-precheck-fix` |
| 模板引用 | `workflows/workflow-template.json` |
| 技能目录 | `workflows/skills-list.json` |
