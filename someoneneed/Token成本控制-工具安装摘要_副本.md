...
## 使用习惯
1. 一个会话处理一个问题。修 Bug 开一个，做重构开一个，写文章开一个，查线上问题开一个。别混着来。Session 越长 → 继承的历史越多 → 每一轮越贵。一个 Session 只服务一个目标。Topic 切分就是最便宜的 Context 管理。而且最好一次性说完。对话式沟通的每一轮来回都在重新组装上下文、重新带入历史、重新确认状态。四轮对话的成本，往往是一次完整表达的好几倍。
2. CO-STAR
3. 引用文件时带完整路径。
4. 很多 Agent 已经支持自动压缩，但自动压缩通常依赖上下文水位，只是解决容量问题。阶段完成、决策收敛、任务状态这些主动压缩的判断还是得人自己决定，至少目前是这样的。`/compact` 做的，本质上就是把完整历史压成可继续工作的状态。
5. 在很多日常任务里，一句指令就够了：直接给结论，不要复述问题，必要时再展开。参考项目 [Caveman](https://github.com/juliusbrussee/caveman)。更进一步，把输出格式也一并说清楚：只要 diff / 步骤 / 结论 / 表格 / JSON，直接要求。输出更短 + 结果更可用 → 重试更少，一次约束省两类成本。
6. CLI 优先于 MCP。能用 CLI 解决 → 尽量用 CLI，能用脚本解决 → 尽量用脚本，最后才考虑 MCP。MCP 的价值在于：跨系统编排、结构化返回、权限统一收敛、内部业务能力包装。
7. Skill / Agent / Command 都应该绑定模型。模型路由不该只存在于“主对话框”的心智里，要落实到执行单元。Skill 绑定模型：写 UT、commit、格式修复、样板代码，明确绑定便宜模型。`SKILL.md` 文件头部可以直接声明模型和上下文策略：`model` 字段指定该 Skill 使用的模型，`context: fork` 表示把这类固定流程放到独立执行上下文里，适合任务边界清晰、可单独完成的 Skill。这样一来，低复杂度的 Skill 就能自动使用便宜模型，也不必把主会话的全部历史带进去。
```yaml
---
context: fork
...
---
```
8. 先用便宜模型，再升级。很多任务不需要一上来就用最贵的模型。更省的模式可以简化成一条升级链路：便宜模型先跑 → 判断复杂度 → 需要时才升级。例如：先让便宜模型分类（重构？Bug？文档？），先做初筛（PR 有没有高风险？），先做摘要（20 页 → 1 页），再交给强模型做推理。这种级联工作流既省钱，又让贵模型花在刀刃上。
9. Agent 绑定模型：Planner → 中高档; Coder  → 中档 / 便宜;Reviewer → 强
10. 典型场景
| 任务            | 推荐模型档位 | 理由             |
| --------------- | ------------ | ---------------- |
| 架构设计        | 强模型       | 长链路推理       |
...
### 可以写到Agents.md
1. 重写模块的效果不如新增再替换。 
2. 引导自动化测试。如果你的项目没有自动化测试：请帮我初始化自动化测试。如果你的项目已经有了，在 `AGENTS.md` 里面加上：实现功能的同时，要同步写自动化测试代码，并且完成后要用测试代码验证。
3. 报告（测试报告、分析报告、汇总报告）用 HTML 比 Markdown 更好，而且能引入 ECharts 的 CDN 来绘制图表，更自由、更直观。
4. 如果是 AI 生成的 PRD 文档，一定要让它输出一份言简意赅、小孩都能看懂的简报。特别是使用了 `grill-me` 这类技能，一直接受、同意，最后生成的需求文档可能连自己都看不懂。
5. 注意你的主要任务是分析、编排和验证，具体任务尽可能交给 subagent（Opus）去执行。当主 agent 是 Fable 5 时尤其如此：自己只做需求澄清、方案拆解、任务分发和结果验收，实现类工作（读大量代码、写代码、跑测试、批量修改）一律用 Agent 工具派给 Opus subagent 执行。


## 涉及工具
1. 私有数据接个 MCP 就可以，比如写一个 Codex 插件能读取私有数据，那么 Codex 就可以访问这些私有数据，做很多事。
2. 最好装浏览器插件 `BrowserSkill`，直接用浏览器跑测试，会松快很多。
...
## 2. Caveman（4.2）— AI 回复输出压缩
Caveman 压的是 AI 回复

作用：压缩 AI 回复（输出 Token），平均节省 65–75%。四档：`lite` / 默认 full / `ultra` / `wenyan`。

安装（CodeBuddy 用 studyzy Fork 版，上游无 CodeBuddy 支持）：
...
输入 `/caveman` 激活，模式在会话内持续生效，输入 `stop caveman` 关闭
---

## 3. headroom（4.3）— 全量上下文压缩
...
## 4. context-mode（4.4）— MCP 工具输出压缩 + 会话连续性

作用：解决 MCP 工具返回值过大（如 Playwright 快照 56KB）。四能力：Sandbox 工具输出（98% 压缩，315KB→5.4KB）、跨 `/compact` 会话连续性（本地 SQLite + FTS5）、`ctx_execute()` 用代码代替读文件、不干预输出格式。
...
## 5. Graphify（5.2）— 代码知识图谱（Skill 形态）
...
在 CodeBuddy 里执行 `/graphify` 扫描项目。支持 30+ 语言，git hook 增量更新。

---

## 6. CodeGraph（5.3）— 持久化图数据库（MCP 形态）
