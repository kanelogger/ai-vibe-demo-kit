...
工具、常驻规则和工作流检查都有维护成本。先用一个真实任务建立基线，记录耗时、Token、人工纠正、失败重试和最终验收结果，再判断哪一处值得增加机制。任务中的重复找资料、重复操作或反复漏验，才是具体的改进入口。

遇到失败时，沿任务输入、上下文、工具调用和验证结果查找第一个偏离预期的位置。缺入口就补入口，操作路径稳定且反复出现时再沉淀为 Skill 或脚本，能够机械判断的失败再接入门禁。每次尽量只改一个因素，用原来的失败场景和一条正常路径确认修复有效、没有误拦。

引入之后，也要给机制留下调整或退出的条件：

| 观察到的信号 | 处理方式 |
| --- | --- |
| 持续减少重复读取、人工纠正或返工，验收质量保持稳定 | 保留，并记录适用范围 |
| 只有少数任务用到，却每次都加载 | 缩小触发范围，改为按需读取或调用 |
| 多处规则表达同一要求，多个工具重复处理同一材料 | 合并到一个维护入口，移除重复环节 |
| 正常任务频繁被拦截，解释和绕行成本增加 | 检查条件是否过宽，用正常与失败场景修正边界 |
| 长期无人使用，或额外调用、维护和返工成本超过收益 | 核实依赖后停用，并清理入口与配置引用 |

长期没有触发的权限、数据完整性和不可逆操作保护，仍需按其保护目标判断价值。调整这类检查时，要确认原有边界继续受到保护。

每项机制能说清解决过什么问题、在哪些任务中有效、出现什么信号时该调整，就足以支持后续维护。这些信息放在已有配置说明或操作记录里即可。模型和项目变化后，继续用任务结果判断收益，避免把曾经有用的配置永久保留下来。
