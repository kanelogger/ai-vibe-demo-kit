...
export function formatUpgradeCommand(packageVersion, root) {
  if (!SEMVER.test(packageVersion ?? "")) fail("E_DISTRIBUTION_MANIFEST", "Distribution version cannot produce a safe upgrade command");
  return `npx --yes ${shellQuote(`${PACKAGE_NAME}@${packageVersion}`)} upgrade --target ${shellQuote(root)} --apply --json`;
}
