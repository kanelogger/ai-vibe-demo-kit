...
* Implement `kit check` as a lightweight framework that reads `workflow-state.json`, validates its JSON schema, and checks that `allowedNextStages` matches the transition table from `00-contract.md` (terminal stage `implementation-ready` uses `[]`). Specific artifact existence checks per stage are added in Phase 2.
...
* `kit check` reads `workflow-state.json` and validates: JSON schema integrity, `stage` is a known value, `allowedNextStages` must be an array and match the transition table in `00-contract.md` (only terminal stage `implementation-ready` may be `[]`).
...
* Manually create a minimal `workflow/requirements.md` with YAML frontmatter `status: draft`. This satisfies `kit stage advance`'s pre-advance artifact validation per `00-contract.md`.
