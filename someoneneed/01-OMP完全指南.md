# OMP 完全指南：从安装到会话、上下文与自主模式

OMP（oh-my-pi）是 Pi Coding Agent 的增强 Fork。

它保留了终端 Agent 的基本交互方式，并把模型路由、会话树、上下文压缩、计划模式、目标模式、子 Agent、插件和调试工具整合进同一个 TUI。

是我目前最主要的 coding agent.

## 一、十分钟跑通 OMP

### 1. 安装

OMP 提供 Bun 包、安装脚本和 mise 三种常见安装方式。任选一种即可，最终都会在 `PATH` 中生成 `omp`：

| 方式 | 命令 | 适合谁 |
| --- | --- | --- |
| Bun | `bun install -g @oh-my-pi/pi-coding-agent` | 已安装 Bun 1.3.14 或更高版本 |
| 安装脚本 | `curl -fsSL https://raw.githubusercontent.com/can1357/oh-my-pi/main/scripts/install.sh \| sh` | 希望自动选择 Bun 或预编译二进制 |

安装后先验证三件事：

```sh
omp --version           # 当前二进制版本
omp config path         # 生效的 agent 配置目录
omp -p 'hello'          # 一次性调用，不进入 TUI
```
...
其他常见变量包括 `OPENAI_API_KEY`、`GEMINI_API_KEY`、`OPENROUTER_API_KEY`、`XAI_API_KEY`、`GROQ_API_KEY`、`MISTRAL_API_KEY` 和 `ZAI_API_KEY`。
完整映射见[环境变量文档](https://omp.sh/docs/env)。

`/login` 适合需要 OAuth 的订阅提供商：

```text
/login
```

`/login` 会追加凭据；`/logout` 用于撤销。保存的凭据位于 `~/.omp/agent/agent.db`。迁移设备时，这个数据库比 `config.yml` 更容易被遗漏。
...
```text
概括 src/main.ts 的职责，只读，不修改文件。
```

需要在脚本或 CI 中执行单轮任务时，使用 Print 模式：

```sh
omp -p "列出 src 下的 TypeScript 文件"
```

结构化集成还可使用 JSON、RPC 或 ACP 模式：

| 模式 | 调用方式 | 用途 |
| --- | --- | --- |
| Interactive | `omp` | 日常开发的 TUI |
| Print | `omp -p "prompt"` | shell、脚本、CI |
| JSON | `omp --mode json -p "prompt"` | 换行分隔的结构化事件 |
| RPC | `omp --mode rpc` | 程序化客户端 |
| ACP | `omp --mode acp` | 支持 Agent Client Protocol 的宿主 |
...
| 命令 | 用途 |
| --- | --- |
| `/new` | 保留旧会话，开始一个干净的新任务 |
| `/drop` | 删除当前会话并新建 |
| `/rename <title>` | 设置容易识别的会话名称 |
| `/move <path>` | 把会话重新绑定到另一个工作目录 |
| `/session info` | 查看 ID、路径、谱系和统计信息 |
| `/resume` | 打开当前项目的会话选择器 |
