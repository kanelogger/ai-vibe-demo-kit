...
const form = reactive<PostPayload & { id?: number }>({
  postCode: "",
  postName: "",
  status: 1,
  description: ""
...
function asPost(row: unknown): PostItem {
  return row as PostItem;
}
...
async function loadRows() {
  ...
  try {
    ...
    message(resolveError(error, "岗位列表加载失败"), { type: "error" });
  } finally {
    loading.value = false;
  }
}
...
async function submitForm() {
  ...
  await formRef.value.validate(async valid => {
    ...
    try {
      ...
      if (dialogMode.value === "create") await createPost(payload);
      else if (form.id) await updatePost(form.id, payload);
...
async function handleStatusChange(row: PostItem) {
  const nextStatus = row.status === 1 ? 0 : 1;
  try {
    await updatePostStatus(row.id, nextStatus);
...
async function handleDelete(row: PostItem) {
  try {
    await ElMessageBox.confirm(`确认删除岗位 ${row.postName}？`, "删除确认", {
      type: "warning",
    ...
    <el-card shadow="never" class="search-panel">
      <el-form :model="query" inline label-width="72px">
        <el-form-item label="岗位编码">
          <el-input v-model="query.postCode" clearable placeholder="岗位编码" />
        </el-form-item>
        <el-form-item label="岗位名称">
          <el-input v-model="query.postName" clearable placeholder="岗位名称" />
        </el-form-item>
    ...
    <el-card shadow="never" class="table-panel">
      <el-table v-loading="loading" :data="rows" row-key="id">
        ...
        <el-table-column label="操作" fixed="right" width="180">
          <template #default="{ row }">
            <el-button link type="primary" @click="openEdit(asPost(row))">
              编辑
            </el-button>
            <el-button link type="primary" @click="handleStatusChange(asPost(row))">
              {{ row.status === 1 ? "停用" : "启用" }}
            </el-button>
            <el-button link type="danger" @click="handleDelete(asPost(row))">
              删除
            </el-button>
          </template>
        </el-table-column>
