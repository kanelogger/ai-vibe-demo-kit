...
Version changes must be committed before this script runs. An already published version fails closed.
EOF
}
...
if view_output="$(npm view "${package_name}@${package_version}" version --registry="$REGISTRY" 2>&1)"; then
  fail "${package_name}@${package_version} is already published. Commit a complete version update before retrying."
elif ! grep -Eqi 'E404|404 Not Found' <<<"$view_output"; then
  fail "Could not check npm version availability: ${view_output}"
fi

printf 'Registry version is available.\n'
