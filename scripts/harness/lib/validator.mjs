...
export function validateEnvironmentManifest(value) {
  ...
  for (const heading of ENVIRONMENT_HEADINGS) {
    if (!value.split("\n").includes(heading)) {
      errors.push(issue("E_ENVIRONMENT_SECTION", heading, `required section is missing: ${heading}`));
    }
  }
  ...
  if (placeholderLines.length > 0) {
    errors.push(issue("E_ENVIRONMENT_PLACEHOLDER", `line.${placeholderLines[0]}`, `${placeholderLines.length} line(s) contain unresolved fill placeholders`));
  }
  if (incompleteChecklistLines.length > 0) {
    errors.push(issue("E_ENVIRONMENT_CHECKLIST", `line.${incompleteChecklistLines[0]}`, `${incompleteChecklistLines.length} alignment checklist item(s) are not confirmed`));
  }
  ...
  if (invalidCapabilityStatusLines.length > 0) {
    errors.push(issue("E_ENVIRONMENT_CAPABILITY_STATUS", `line.${invalidCapabilityStatusLines[0]}`, `${invalidCapabilityStatusLines.length} capability row(s) use an invalid status`));
  }
