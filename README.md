...
完成条件：
1. GIVEN <前置条件> WHEN <用户动作> THEN <用户可观察的结果>
2. GIVEN <兼容场景> WHEN <原有操作> THEN <行为保持不变>
3. GIVEN <测试前置> WHEN <运行命令> THEN <测试或真实操作通过>
...
`--done-when` 仍是字符串接口，但建议每条条件都写成一条可验证的 `GIVEN -> WHEN -> THEN`，并在交付说明中逐条列出证据：

```text
GIVEN 当前筛选条件已设置
WHEN 用户点击导出
THEN 生成的 CSV 只包含当前筛选命中的订单
证据：node --test test/export.test.mjs
```

合格条件只表达一个判断。下面这种写法需要拆开：

```text
功能完成且性能良好且兼容旧行为
```

如果条件无法由命令或测试验证，明确写成“人工验收：……”并在交付报告中保留用户检查结果。Harness 不解析自然语言，也不会替 Agent 判断完成条件是否真实满足。
