...
通过单一 command Interface 解析 Profile/Workflow 选择、从 `source/workflows/` 加载 Workflow、投影状态、计算 Next Actions、处理幂等 Signal、绑定漂移、完成资格和领域错误。
...
| `selection.mjs` | Profiles 校验、Profile→Workflow 选择与 Active binding digest |
...
| `validation/workflow.mjs` | Workflow、Catalog 与 Skill entity |
...
- Profile 引用完整 Workflow；Runtime 不合并 `skillCalls`。
- Required Skill 未就绪禁止 `start`；`signal`/非 abort `decide` 重新验证 Active binding，abort 在漂移下仍可用。
- `status` 只读观测 bindingDrift/bindingIssues，不修复、不物化、不改写状态。
