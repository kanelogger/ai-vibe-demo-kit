...
`project.yml#architecture_memory.code_roots` 是代码目录范围的唯一来源。`.agents/`、`source/`、`work/` 等项目治理目录不属于代码模块，除非它们被显式配置为代码根目录。

在代码根目录内：

1. 每个未被 `exclude` 排除的文件夹都是一个模块，并且必须包含 `ARCHITECTURE.md`。
2. 开始修改前，读取相关模块及其父级模块的 `ARCHITECTURE.md`。
3. 新建文件夹时，从 `source/knowledge/templates/architecture-template.md` 创建其 `ARCHITECTURE.md`，并在同一次变更中更新直接父模块的子模块索引。
