...
const goldenRoot = join(sourceRoot, "scripts", "harness", "test", "fixtures", "goldens");
...
test("doctor JSON matches Runtime and completion tooling conflict goldens", async () => {
  ...
  await unlink(join(toolingTarget, "workflows", "stage-result-template.json"));
...
test("Distribution status and exit mappings match the stable golden", async () => {
  ...
  await writeFile(join(target, "AGENTS_template.md"), "seed drift\n");
  result = await runCli(target, ["init"]);
