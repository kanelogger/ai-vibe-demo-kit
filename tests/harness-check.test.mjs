...
// 在 valid-context 夹具上布置一套已同步的外部 Skill：清单、锁文件、目录与 .gitignore。
...
async function plantManagedSkill(root, name) {
  await mkdir(join(root, ".agents", "skills", name), { recursive: true });
  await writeFile(join(root, ".agents", "skills", name, "SKILL.md"), `---\nname: ${name}\n---\n# ${name}\n`, "utf8");
}

async function plantSyncedState(root, { managed = ["ext-skill"], extraSources = [], gitignore = true } = {}) {
  const sources = [EXTERNAL_SOURCE, ...extraSources];
  await writeFile(
    ...
    JSON.stringify(
      {
        ...
        managed: managed.map((name) => ({ name, ...EXTERNAL_SOURCE, resolved: EXTERNAL_SOURCE.ref })),
      },
      null,
      2,
    ),
  ...
  for (const name of managed) await plantManagedSkill(root, name);
  if (gitignore) {
    await writeFile(
      join(root, ".agents", "skills", ".gitignore"),
      ["# Generated by scripts/skills-sync.mjs — managed external skills, do not edit.", ...managed.map((name) => `/${name}/`), ""].join("\n"),
      "utf8",
    );
  }
}

test("context: 外部 Skill 清单、锁文件与磁盘一致时通过", async () => {
  const root = await mutableCopy("valid-context");
  try {
...
test("context: 受管 Skill 缺失、来源未同步、锁文件过期、gitignore 缺项分别报稳定 ID", async () => {
  const root = await mutableCopy("valid-context");
  try {
    await plantSyncedState(root, {
      managed: ["ext-skill", "ghost-skill"],
      extraSources: [{ repo: "https://example.com/team/other", path: ".", ref: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" }],
      gitignore: false,
    });
    await rm(join(root, ".agents", "skills", "ext-skill"), { recursive: true, force: true });
    // ghost-skill 的锁文件条目与任何来源都不匹配：把它的 ref 改掉。
    const lockPath = join(root, ".agents", "skills.lock.json");
    const lock = JSON.parse(await readFile(lockPath, "utf8"));
    lock.managed.find((entry) => entry.name === "ghost-skill").ref = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
    ...
    assert.ok(ids.includes("context.skill-lock-stale"), ids.join(","));
...
test("context: 声明来源但缺锁文件报错", async () => {
  const root = await mutableCopy("valid-context");
  try {
    await writeFile(
      join(root, ".agents", "skills.sources.json"),
      JSON.stringify({ version: 1, skillsRoot: ".agents/skills", sources: [EXTERNAL_SOURCE] }, null, 2),
      "utf8",
    );
    const result = runCheck(root, "context");
    assert.equal(result.code, 1);
    assert.ok(errorIds(result.stdout).includes("context.skill-lock-missing"));
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
